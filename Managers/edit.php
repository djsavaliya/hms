<?php
	...
	$errors=array('fname'=>'','lname'=>'','email'=>'', 'password'=>'','cpassword'=>'','mobile'=>'','dob'=>'');
	...
	if(isset($_POST['edit']))
	{
		...
		if($valid)
		{
			if($conn)
			{
				$sql="UPDATE managers SET first_name = '$fname', last_name = '$lname', date_of_birth = '$dob', mobile = '$mobile', password = '$password' WHERE manager_id = '$manager_id'";
			}
...
<html>
	...
	<h4 class="center heading-text">Edit Manager Profile</h4>
	<form class="white" action="edit.php" method="post">
    
        <label><h5>First Name: </h5></label>
        <input type="text" name="fname" value="<?php echo $manager['first_name'] ?>">
        <div class="red-text"><?php echo $errors['fname']; ?></div>
      
        <label><h5>Last Name: </h5></label>
        <input type="text" name="lname" value="<?php echo $manager['last_name'] ?>">
        <div class="red-text"><?php echo $errors['lname']; ?></div>

      	<label><h5>Date of Birth: </h5></label>
        <input type="date" name="DOB" value="<?php echo $manager['date_of_birth'] ?>">
	    <div class="red-text"><?php echo $errors['dob']; ?></div>

      	<label><h5>Email: </h5></label>
      	<input type="email" name="email" value="<?php echo $manager['email'] ?>" disabled>
        <div class="red-text"><?php echo $errors['email']; ?></div>

      	<label><h5>Mobile: </h5></label>
      	<input type="text" name="mobile" value="<?php echo $manager['mobile'] ?>">
